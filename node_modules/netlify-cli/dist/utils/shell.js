...
const isCommandResult = (value) => typeof value === 'object' &&
    value !== null &&
...
export const canReportMissingCommandName = (command) => !/(?:&&|\|\||[|;<>])/.test(command) && !/^\s*[\w.-]+=/.test(command);
export const shouldUseShell = (command) => /(?:&&|\|\||[|;<>])/.test(command) || /^\s*[\w.-]+=(?:"[^"]*"|'[^']*'|\S+)\s+\S/.test(command);
const isMissingCommandMessage = ({ command, output }) => output.split(/\r?\n/).some((line) => {
...
export const isNonExistingCommandError = ({ command, error: commandError }) => {
